...
class AddModel extends CI_Controller
{
	...
	public function index($product_id = 0)
	{
		...
		if(intval($product_id) > 0)
		{
			...
			$data['vehicle'] = $this->addmodel->getvehicle();
		    $data['get_model'] = $this->addmodel->getModel();
		    $data['list'] = $this->addmodel->get_modelDetails($product_id);
		    // print_r($data['model_details']);
		    // exit;
